...
A self-hosted GitLab bot that reviews merge requests using a two-model council: Codex and Claude run independent reviews in parallel, then a synthesis model merges results into one review result. That result is published as inline GitLab diff discussions where possible, with top-level fallback discussions for findings that cannot be anchored to the diff.
...
GitLab webhook
  ...
  worker.py       ← thread pool, polls queue, calls service
      │
  service.py      ← orchestration: checkout → review/reconcile → publish inline discussions
...
6. Full review:
   ...
   - bot publishes inline discussions, human-thread replies, or top-level fallback discussions
...
- Findings have their own tracked GitLab discussions (inline or top-level).
...
- trigger conditions for full review and discussion reconcile
- "Review prompt" section — what sections are included in the prompt
